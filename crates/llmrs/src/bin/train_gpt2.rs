//! Educational port of `train_gpt2.c` to Rust.
//!
//! The binary drives a complete GPT-2 training loop: it loads checkpoints and datasets,
//! runs forward/backward passes with the original tensor layout, and applies AdamW to
//! update the model. Naming closely mirrors the C version (B, T, C, L, NH, V, etc.)
//! so the source can be compared line by line with the reference implementation.
